	...
	for _, metricSpec := range hpa.Spec.Metrics {
		...
		case autoscalingv2.ExternalMetricSourceType:
			...
			if metricSpec.External.MetricSelector != nil {
				...
				if metricSpec.External.MetricSelector.MatchLabels != nil {
					em.CustomTags = extractCustomTags(em.Labels, hpa.Annotations)
				}
			}
			// Look for a custom aggregation function.
...
func extractCustomTags(metricLabels map[string]string, hpaAnnotations map[string]string) map[string]string {
	...
	for name, value := range metricLabels {
		...
		if hpaAnnotations == nil {
			continue
		}
		if tagValue, ok := hpaAnnotations[value]; ok {
			customTags[value] = tagValue
		} else {
			log.Warnf("Label selector '%s:%s' is a custom tag reference but no corresponding annotation was found!", name, value)
		}
	}
	return customTags
}
